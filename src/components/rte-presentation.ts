...
export class RTEPresentationElement extends LitElement {
	...
	render() {
		return html`
			...
			<div id="rte-readonly" class="rte-renderer">
				<div id="json-overlay" class="rte-renderer ${this.showJSON ? '' : 'hide'}">
					<pre>
			...
			<button id="json-toggle" @click=${() => (this.showJSON = !this.showJSON)}>Toggle JSON</button>
		`;
	}
